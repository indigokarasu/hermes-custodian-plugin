...
def _summarize_journal(data: Dict) -> Dict:
    ...
    Handles the current escalation-runner format (run_id/timestamp/fixes_applied/
    issues_resolved/system_health) and the legacy entries/kind format.
    """
    # Current escalation-runner format
    if "system_health" in data or "issues_resolved" in data or "fixes_applied" in data:
        fixes = data.get("fixes_applied") or []
        resolved = data.get("issues_resolved") or []
        still_open = data.get("issues_verified_still_open") or []
        return {
            "run_id": data.get("run_id", ""),
            "created_at": data.get("timestamp", "") or data.get("created_at", ""),
            "issues_found": len(still_open),
            "fixes_applied": len(fixes) + len(resolved),
            "escalations": int(data.get("escalated_issues", 0) or 0),
            "has_actions": bool(fixes or resolved),
            "has_escalations": bool(data.get("escalation_needed", False)),
            "summary": data.get("summary", ""),
        }
    # Legacy entries/kind format
        ...
        if date_dirs:
            ...
            if files:
                try:
                    ...
                    last_scan_at = data.get("created_at") or data.get("timestamp")
                    if last_scan_at:
                        try:
...
def _get_system_counts() -> Dict[str, Any]:
    ...
    cron_disabled = 0
    cron_never_run = 0
    # jobs.json lives under the active profile; fall back to the home-level path.
    active = "default"
    try:
        ap = hermes_home / "active_profile"
        if ap.exists():
            active = ap.read_text(encoding="utf-8").strip() or "default"
    except Exception:
        pass
    job_candidates = [
